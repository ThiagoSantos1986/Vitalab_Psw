...
def solicitar_exames(request):
    ...
    if request.method == "GET":
     ...
     # retona uma lista com os valores setados else:
        exames_id = request.POST.getlist('exames')

        solicitacao_exames = TiposExames.objects.filter(id__in=exames_id)
        #preco_total = solicitacao_exames.aggregate(total=Sum('preco'))['total']
        #todo calcular preco dos dados disponiveis os exames disponiveis
        preco_total = 0
...
def fechar_pedido(request):
    ...
    for exame  in solicitacao_exames:  # criando a instancia para salvar na tabela do db
...
def gerenciar_pedidos(request):
    pedidos_exames = PedidosExames.objects.filter(usuario=request.user)  #busca no db os pedidos conforme id do usuario logado
...
def acesso_medico(request, token):
    ...
    if acesso_medico.status == "Expirado":
        messages.add_message(request, constants.ERROR, 'Esse token expirou solicite outro.')
