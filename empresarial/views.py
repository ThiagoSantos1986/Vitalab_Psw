...
@staff_member_required # forca so membros da equipe ter acesso.
...
def alterar_dados_exame(request, exame_id):
    ...
    pdf = request.FILES.get('resultado')# capturando o arquido do html
    ...
    messages.add_message(request, constants.SUCCESS, 'Sucesso na modificação')
    return redirect(f'/empresarial/exame_cliente/{exame_id}')